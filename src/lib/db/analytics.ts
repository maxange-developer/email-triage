...
export async function getVolumeByDayByAccount(accountId: string, days = 30): Promise<DayCount[]> {
  ...
  if (error) throw new Error(`getVolumeByDayByAccount: ${error.message}`)
...
export async function getCategoryBreakdownByAccount(accountId: string, days = 30): Promise<CategoryCount[]> {
  ...
  const { data, error } = await db
    ...
    .gte('received_at', sinceDate(days))
  if (error) throw new Error(`getCategoryBreakdownByAccount: ${error.message}`)
...
export async function getTopSendersByAccount(accountId: string, days = 30, limit = 10): Promise<SenderRow[]> {
  ...
  const { data, error } = await db
    ...
    .eq('account_id', accountId)
    .gte('received_at', sinceDate(days))
  if (error) throw new Error(`getTopSendersByAccount: ${error.message}`)
  ...
  for (const row of data ?? []) {
    ...
    if (!existing) {
      ...
      senderMap.set(addr, { from_name: row.from_name as string | null, count: 1, priorityCounts })
    } else {
      existing.count++
      const priority = row.priority as string | null
      if (priority) existing.priorityCounts.set(priority, (existing.priorityCounts.get(priority) ?? 0) + 1)
    }
  }
  ...
  return Array.from(senderMap.entries())
    ...
    .map(([from_address, info]) => {
      ...
      for (const [priority, cnt] of info.priorityCounts.entries()) {
        if (cnt > maxCount) { maxCount = cnt; top_priority = priority }
      }
      return { from_address, from_name: info.from_name, count: info.count, top_priority }
    })
}
...
export async function getAnalyticsSummaryByAccount(accountId: string, days = 30): Promise<AnalyticsSummary> {
  ...
  const { data, error } = await db
    ...
    .eq('account_id', accountId)
    .gte('received_at', sinceDate(days))
  if (error) throw new Error(`getAnalyticsSummaryByAccount: ${error.message}`)
